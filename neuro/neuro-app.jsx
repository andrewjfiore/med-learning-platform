...
const MODULES = [
  ...
  { id: "tract3D", name: "3D Explorer", icon: "🧊" },
  { id: "strokeTrainer", name: "Stroke Trainer", icon: "🩺" },
  { 
// ─── Tract ID Trainer ──────────────────────────────────────────────────────────
const TractIDTrainer = ({ addXp }) => {
  ...
  const LEVELS = {
    cervical: { name: "Cervical", img: "img/sc-cervical-cross.svg", tracts: ["dorsalColumns", "lateralCST", "spinothalamic", "dorsalSCT", "rubrospinal", "vestibulospinal", "medialReticulospinal"], desc: "Largest cord area. Thick white matter, prominent ventral horns." },
    thoracic: { name: "Thoracic", img: "img/sc-thoracic-cross.svg", tracts: ["dorsalColumns", "lateralCST", "spinothalamic", "dorsalSCT", "lateralReticulospinal"], desc: "Smaller. Lateral horn present (sympathetic)." },
    lumbar: { name: "Lumbar", img: "img/sc-lumbar-cross.svg", tracts: ["dorsalColumns", "lateralCST", "spinothalamic", "ventralSCT", "vestibulospinal"], desc: "Enlargement for lower limb. Large ventral horns." },
    sacral: { name: "Sacral", img: "img/sc-all-levels-wellcome.jpg", tracts: ["spinothalamic", "ventralSCT"], desc: "Smallest. Mostly gray matter." }
  };
  
  const generateQuestion = () => {
    const lvlKeys = Object.keys(LEVELS);
    const lvl = lvlKeys[Math.floor(Math.random() * lvlKeys.length)];
    const levelData = LEVELS[lvl];
    const target = levelData.tracts[Math.floor(Math.random() * levelData.tracts.length)];
    const options = levelData.tracts.sort(() => Math.random() - 0.5).slice(0, 4);
    return { level: lvl, target, options };
  };
  
  const [question, setQuestion] = useState(generateQuestion);
  
  const handleSelect = (tid) => { if (!showAnswer) setSelected([tid]); };
  const checkAnswer = () => { setShowAnswer(true); setTotal(t => t + 1); if (selected[0] === question.target) { setScore(s => s + 1); addXp(10, question.target); } };
  const next = () => { setQuestion(generateQuestion()); setSelected([]); setShowAnswer(false); setCurrentQ(q => q + 1); };
  
  const lvl = LEVELS[level];
  
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ fontFamily: FONTS, fontSize: 22, fontWeight: 800, margin: "0 0 16px" }}>🎓 Tract ID Trainer</h2>
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
        {Object.keys(LEVELS).map(k => (
          <button key={k} onClick={() => setLevel(k)} style={{ padding: "8px 14px", borderRadius: 8, border: level === k ? "2px solid #4A7A8A" : "1px solid #ddd", background: level === k ? "#4A7A8A15" : "#fff", color: "#333", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: FONTS }}>{LEVELS[k].name}</button>
        ))}
      </div>
      <div style={{ display: "flex", gap: 16, marginBottom: 16, fontSize: 14, fontWeight: 600, fontFamily: FONTS }}>
        <span style={{ color: "#2E8B57" }}>✅ {score}/{total}</span><span>Question {currentQ + 1}</span>
      </div>
      <div style={{ background: "#fff", borderRadius: 12, padding: 12, marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
        <img src={lvl.img} alt={lvl.name} style={{ width: "100%", maxWidth: 350, borderRadius: 8, display: "block", margin: "0 auto" }} />
        <p style={{ fontSize: 12, color: "#666", marginTop: 8, textAlign: "center", fontFamily: FONTS }}>{lvl.desc}</p>
      </div>
      <p style={{ fontSize: 15, fontWeight: 700, margin: "0 0 12px", fontFamily: FONTS }}>👆 Select the correct tract:</p>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8, marginBottom: 16 }}>
        {question.options.map(tid => {
          const t = TRACTS[tid];
          const sel = selected.includes(tid);
          const corr = showAnswer && tid === question.target;
          const wrong = showAnswer && sel && tid !== question.target;
          return <button key={tid} onClick={() => handleSelect(tid)} disabled={showAnswer} style={{ padding: "14px 10px", borderRadius: 10, border: corr ? "3px solid #2E8B57" : wrong ? "3px solid #C94040" : sel ? `2px solid ${t.color}` : "1px solid #ddd", background: corr ? "#2E8B5715" : wrong ? "#C9404015" : sel ? t.color + "20" : "#fff", color: corr ? "#2E8B57" : wrong ? "#C94040" : "#333", fontSize: 13, fontWeight: 700, cursor: showAnswer ? "default" : "pointer", fontFamily: FONTS }}>{t?.short || tid}</button>;
        })}
      </div>
      {showAnswer && <div style={{ marginBottom: 16, padding: 12, background: selected[0] === question.target ? "#2E8B5715" : "#C9404015", borderRadius: 8, borderLeft: `4px solid ${selected[0] === question.target ? "#2E8B57" : "#C94040"}` }}><p style={{ fontWeight: 700, margin: 0, fontFamily: FONTS }}>{selected[0] === question.target ? "✅ Correct!" : "❌ " + TRACTS[question.target]?.name}</p><p style={{ fontSize: 12, margin: "4px 0 0", fontFamily: FONTS }}>{TRACTS[question.target]?.function}</p></div>}
      <button onClick={showAnswer ? next : checkAnswer} disabled={!showAnswer && selected.length === 0} style={{ width: "100%", padding: "14px 20px", borderRadius: 10, border: "none", background: !showAnswer && selected.length === 0 ? "#ccc" : "#4A7A8A", color: "#fff", fontSize: 15, fontWeight: 700, cursor: !showAnswer && selected.length === 0 ? "not-allowed" : "pointer", fontFamily: FONTS }}>{showAnswer ? "Next Question →" : "Check Answer"}</button>
    </div>
  );
};
...
const TractExplorer3D = ({ addXp }) => {
  ...
  const tractPaths = {
    dorsalColumns: { color: "#5B8A9A", path: "M 55,10 L 55,290", label: "DCML - climbs to medulla" },
    lateralCST: { color: "#7B68AE", path: "M 25,10 L 25,290", label: "LCST - descends" },
    spinothalamic: { color: "#C97070", path: "M 85,50 L 85,290", label: "STT - ascends to thalamus" },
    dorsalSCT: { color: "#6BAF7B", path: "M 40,100 L 40,290", label: "DSCT - to cerebellum" },
    ventralSCT: { color: "#5B9A8A", path: "M 70,180 L 70,290", label: "VSCT" },
    rubrospinal: { color: "#8B6914", path: "M 30,50 L 30,200", label: "RST" },
    vestibulospinal: { color: "#4A8A6A", path: "M 50,100 L 50,290", label: "VeST" },
    medialReticulospinal: { color: "#6B7E23", path: "M 45,50 L 45,290", label: "MeRST" },
  };
  
  return (
    <div style={{ padding: 16 }}>
      <h2 style={{ fontFamily: FONTS, fontSize: 22, fontWeight: 800, margin: "0 0 16px" }}>🧊 3D Isometric Tract Explorer</h2>
      <p style={{ fontSize: 13, color: "#666", marginBottom: 16, fontFamily: FONTS }}>See how tracts course through the cord. Click to highlight.</p>
      <div style={{ background: "#fff", borderRadius: 12, padding: 16, marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
        <svg viewBox="0 0 100 310" style={{ width: "100%", maxWidth: 180, display: "block", margin: "0 auto" }}>
          <ellipse cx="50" cy="10" rx="20" ry="10" fill="#E8DDD0" stroke="#B8A898"/><rect x="30" y="10" width="40" height="280" fill="#F5EDE3" stroke="#B8A898"/><ellipse cx="50" cy="290" rx="15" ry="8" fill="#E8DDD0" stroke="#B8A898"/>
          <text x="5" y="25" fontSize="6" fill="#888">C</text><text x="5" y="100" fontSize="6" fill="#888">T</text><text x="5" y="200" fontSize="6" fill="#888">L</text><text x="5" y="285" fontSize="6" fill="#888">S</text>
          {Object.entries(tractPaths).map(([id, data]) => (
            <g key={id} onClick={() => setHighlighted(h => h === id ? null : id)} style={{ cursor: "pointer" }}>
              <path d={data.path} stroke={data.color} strokeWidth={highlighted === id ? 5 : 2} fill="none" opacity={highlighted === id ? 1 : 0.4} strokeLinecap="round"/>
              {highlighted === id && <text x="88" y={parseInt(data.path.match(/M \d+,(\d+)/)?.[1] || "50")} fontSize="7" fill={data.color} fontWeight="600">{TRACTS[id]?.short}</text>}
            </g>
          ))}
        </svg>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 6 }}>
        {Object.entries(tractPaths).map(([id, data]) => (
          <button key={id} onClick={() => setHighlighted(id)} style={{ padding: "8px 10px", borderRadius: 8, border: highlighted === id ? `2px solid ${data.color}` : "1px solid #eee", background: highlighted === id ? data.color + "15" : "#fff", textAlign: "left", cursor: "pointer", fontFamily: FONTS }}>
            <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: 2, background: data.color, marginRight: 6 }}></span>
            <span style={{ fontSize: 11, fontWeight: 600 }}>{TRACTS[id]?.short}</span>
          </button>
        ))}
      </div>
      {highlighted && <div style={{ marginTop: 16, padding: 12, background: tractPaths[highlighted].color + "15", borderRadius: 10, borderLeft: `4px solid ${tractPaths[highlighted].color}` }}><p style={{ fontWeight: 700, margin: 0, fontFamily: FONTS }}>{TRACTS[highlighted]?.name}</p><p style={{ fontSize: 11, margin: "4px 0 0", color: "#666", fontFamily: FONTS }}>{tractPaths[highlighted].label}</p></div>}
    </div>
  );
};
...
// ─── Stroke Trainer Module ───────────────────────────────────────────────────────
function StrokeTrainer({ addXp }) {
  const [score, setScore] = React.useState(0);
  const [total, setTotal] = React.useState(0);
  const [showAns, setShowAns] = React.useState(false);
  
  const STROKES = [
    { id: "lacunar", name: "Lacunar Stroke", artery: "Small penetrating arteries (lenticulostriate)", deficits: "Pure motor, pure sensory, ataxic hemiparesis", common: "Hypertension, diabetes" },
    { id: "pica", name: "PICA Infarct", artery: "Posterior Inferior Cerebellar Artery", deficits: "Vertigo, nystagmus, dysphagia, hoarseness (lateral medullary)", common: "Wallenberg syndrome" },
    { id: "assa", name: "Anterior Spinal Artery", artery: "Anterior Spinal Artery", deficits: "Bilateral motor, bilateral pain/temp loss below, urinary retention", common: "Spinal cord infarction" },
    { id: "aica", name: "AICA Infarct", artery: "Anterior Inferior Cerebellar Artery", deficits: "Facial pain/temp loss, corneal reflex absent, facial paralysis", common: "Lateral pontine" },
    { id: "pca", name: "PCA Stroke", artery: "Posterior Cerebral Artery", deficits: "Contralateral homonymous hemianopia, memory impairment", common: "Occipital lobe" },
    { id: "mca", name: "MCA Stroke", artery: "Middle Cerebral Artery", deficits: "Contralateral face/arm > leg weakness, aphasia (dominant), neglect (non-dominant)", common: "Most common stroke" },
  ];
  
  const [q, setQ] = React.useState(() => STROKES[Math.floor(Math.random() * STROKES const [sel,.length)]);
  setSel = React.useState(null);
  
  const nextQ = () => {
    setSel(null);
    setShowAns(false);
    setQ(STROKES[Math.floor(Math.random() * STROKES.length)]);
  };
  
  const check = () => {
    setShowAns(true);
    setTotal(t => t + 1);
    if (sel === q.id) { setScore(s => s + 1); addXp(10, sel); }
  };
  
  const opts = STROKES.filter(s => s.id !== q.id).sort(() => Math.random() - 0.5).slice(0, 3).concat(q).sort(() => Math.random() - 0.5);
  
  return React.createElement("div", { style: { padding: 16 } },
    React.createElement("h2", { style: { fontFamily: FONTS, fontSize: 22, fontWeight: 800, margin: "0 0 16px" } }, "🩺 Stroke Location Trainer"),
    React.createElement("div", { style: { display: "flex", gap: 16, marginBottom: 16, fontSize: 14, fontWeight: 600 } },
      React.createElement("span", { style: { color: "#2E8B57" } }, "✅ " + score + "/" + total)
    ),
    React.createElement("div", { style: { background: "#f8f9fa", borderRadius: 12, padding: 16, marginBottom: 16 } },
      React.createElement("p", { style: { fontSize: 15, fontWeight: 700, margin: "0 0 12px" } }, "Which artery is affected given these deficits?"),
      React.createElement("p", { style: { fontSize: 13, color: "#666", margin: "0 0 16px" } }, q.deficits),
      React.createElement("div", { style: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 } },
        opts.map(s => 
          React.createElement("button", { key: s.id, onClick: () => !showAns && setSel(s.id), disabled: showAns,
            style: { padding: "12px 10px", borderRadius: 10, border: sel === s.id && !showAns ? "2px solid " + (s.id === q.id ? "#2E8B57" : "#C94040") : "1px solid #ddd",
              background: showAns && s.id === q.id ? "#2E8B5715" : showAns && sel === s.id ? "#C9404015" : sel === s.id ? "#4A7A8A20" : "#fff",
              color: showAns && s.id === q.id ? "#2E8B57" : "#333", fontSize: 12, fontWeight: 600, cursor: showAns ? "default" : "pointer", fontFamily: FONTS },
            s.name)
        )
      ),
      showAns && React.createElement("div", { style: { marginTop: 12, padding: 12, background: sel === q.id ? "#2E8B5715" : "#C9404015", borderRadius: 8 } },
        React.createElement("p", { style: { fontWeight: 700, margin: 0 } }, sel === q.id ? "✅ Correct!" : "❌ " + q.name),
        React.createElement("p", { style: { fontSize: 12, margin: "4px 0 0" } }, q.artery)
      )
    ),
    React.createElement("button", { onClick: showAns ? nextQ : check, disabled: !showAns && !sel,
      style: { width: "100%", padding: "14px 20px", borderRadius: 10, border: "none", background: !showAns && !sel ? "#ccc" : "#4A7A8A", color: "#fff", fontSize: 15, fontWeight: 700, cursor: !showAns && !sel ? "not-allowed" : "pointer", fontFamily: FONTS } },
      showAns ? "Next Question →" : "Check Answer"
    )
  );
}
