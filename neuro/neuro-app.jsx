  ...
  const levels = [
    { id: "overview", label: "All Levels", img: "img/sc-all-levels-wellcome.jpg", 
      ...
      regions: [
        ...
        { id: "upper-thoracic", label: "Upper Thoracic", y: "28%", h: "12%", color: "#7B68AE", desc: "T1-T6: Small round, lateral horn (sympathetic), narrow ventral horns" },
        { id: "lower-thoracic", label: "Lower Thoracic", y: "42%", h: "12%", color: "#C94040", desc: "T7-T12: Similar to upper, smaller ventral horns" },
        { id: "lumbar", label: "Lumbar", y: "58%", h: "18%", color: "#2E8B57", desc: "L1-S2: Large enlargement for legs, massive ventral horns" },
        { id: "sacral", label: "Sacral", y: "80%", h: "15%", color: "#E8A838", desc: "S3-Co1: Small, mostly gray matter, autonomic nuclei" },
      ]},
  ...
  return (
    <div style={{ marginTop: 20 }}>
      ...
      <div style={{ background: "#fff", borderRadius: 14, padding: 12, boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        <div style={{ position: "relative", display: "inline-block", width: "100%", maxWidth: current.id === "overview" ? 500 : 400 }}>
          <img src={current.img} alt={current.label}
            style={{ width: "100%", borderRadius: 10, display: "block" }} />
          {current.id === "overview" && current.regions && current.regions.map(r => (
            <div key={r.id} onClick={() => onSelect && onSelect(r.id)}
              style={{
                position: "absolute", left: "2%", width: "96%", top: r.y, height: r.h,
                border: sel === r.id ? `3px solid ${r.color}` : `2px dashed ${r.color}88`,
                background: sel === r.id ? r.color + "25" : "transparent",
                borderRadius: 8, cursor: "pointer", transition: "all 0.2s",
                display: "flex", alignItems: "center", justifyContent: "center"
              }}>
              <span style={{ background: r.color, color: "#fff", padding: "3px 10px", borderRadius: 5, fontSize: 10, fontWeight: 700, fontFamily: FONTS }}>
                {r.label}
              </span>
            </div>
          ))}
        ...
        <p style={{ fontSize: 13, color: "#555", textAlign: "center", margin: "10px 0 0", lineHeight: 1.5, fontFamily: FONTS }}>
          {current.regions && sel && current.regions.find(r => r.id === sel) ? current.regions.find(r => r.id === sel).desc : current.desc}
        {current.id === "overview" && (
          <div style={{ fontSize: 11, color: "#888", marginTop: 6 }}>👆 Click a region on the image</div>
        )}
        </p>
      </div>
    </div>
